...
def train(rank, params, shared_model, optimizer):
    ...
    state , info = env.reset() # state is a numpy array of size 1*42*42, in black & white
    ...
    while True: # repeat
        ...
        for step in range(params.num_steps): # going through the num_steps exploration steps
            ...
            entropy = -(log_prob * prob).sum(1,keepdim=True) # H(p) = - sum_x p(x).log(p(x))
            ...
            state, reward, done, excess_values,excess_values_2 = env.step(int(action)) # playing the selected action, reaching the new state, and getting the new reward
            ...
            if done: # if the episode is done:
                ...
                state , info= env.reset() # we restart the environment
