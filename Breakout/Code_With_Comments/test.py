...
def test(rank, params, shared_model):
    ...
    env = create_atari_env(params.env_name ) # running an environment with a video
    ...
    state , info = env.reset() # getting the input images as numpy arrays
    ...
    while True: # repeat
        ...
        state, reward, done,excess_values,excess_values_2= env.step(action[0]) # done = done or episode_length >= params.max_episode_length
        reward_sum += reward
        if done: # printing the results at the end of each part
            ...
            state , info = env.reset() # reinitializing the environment
